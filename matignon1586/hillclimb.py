...
import sys, os, math, random, collections, pickle
...
def full(ch):
    s = 0.0
    for i in range(n):
        s += logp(''.join(ch[max(0, i-W+1):i]), ch[i])
    return s
def kl(cnt):
...
for r in range(restarts):
    ...
    for it in range(iters):
        ...
        old = m[s]
        new = random.choice(AL)
        if new == old: continue
        idx = pos[s]
        touched = sorted({j for i in idx for j in range(i, min(n, i+W))})
        before = sum(logp(''.join(ch[max(0, j-W+1):j]), ch[j]) for j in touched)
        for i in idx: ch[i] = new
        after = sum(logp(''.join(ch[max(0, j-W+1):j]), ch[j]) for j in touched)
