...
(3) Bootstrap: folios resampled with replacement (500 resamples, random.Random(2026)); statistics are
    ...
    in a thousand).  Reported: percentile 95% interval and bootstrap SE for h2 (raw, no spaces), Zipf
...
def write_md(R):
    ...
    L.append("| cell | folios | tokens | h1 | h2 | mean wlen | hapax % types | Zipf slope (ranks) | -edy % | qo- % | first 10k: h2 / hapax % / Zipf |\n|---|---|---|---|---|---|---|---|---|---|---|")
    for k, d in R["per_hand"].items():
        ...
        L.append(f"| {k} | {d['folios']} | {d['tokens']} | {f3(d['h1'])} | {f3(d['h2'])} | {d['mean_word_len']:.2f} | {100 * d['hapax_type_frac']:.1f} | "
                 f"{f3(d['zipf_slope_1_1000'])} ({d['ranks_used']}) | {100 * d['edy_rate']:.2f} | {100 * d['qo_rate']:.2f} | {tt} |")
    L.append("")
    ...
    L.append("| subset | units | statistic | point | boot mean | SE | 95% CI |\n|---|---|---|---|---|---|---|")
    for name, b in R["bootstrap"].items():
        ...
        for k, lab in [("h2", "h2 raw no-space"), ("zipf_slope", "Zipf slope 1-1000"), ("hapax_type_frac", "hapax type fraction"),
            ...
            if k not in bb:
                continue
            p = pt.get(k)
            L.append(f"| {name} | {bb['n_units']} | {lab} | {f3(p, 4)} | {f3(bb[k]['mean'], 4)} | {f3(bb[k]['se'], 4)} | [{f3(bb[k]['ci95'][0], 4)}, {f3(bb[k]['ci95'][1], 4)}] |")
    L.append("")
    ...
    L.append("")
    L.append("Stratified-by-hand permutation can only exchange labels inside hand 3 (all other hands write one language), so its p-values test whether hand 3's A folios differ from hand 3's B folios; 'hand3_A_vs_B' is the same comparison run directly. 'B_hand2_vs_hand3' asks whether two scribes writing the same language differ.\n")
    ...
    L.append("| sample | tokens | plug-in I | shuffled I | excess | plug-in ratio | held-out H(w) | held-out H(w'|w) | I_ho | ratio_ho | I_ho shuffled | glyph I | glyph I shuffled | glyph excess | glyph ratio |\n|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|")

    def prow(label, w, g):
        return (f"| {label} | {w['tokens']} | {f3(w['plugin']['I'])} | {f3(w['plugin_shuffled']['I'])} | {f3(w['plugin_excess_I'])} | {f3(w['plugin']['ratio'])} | "
                f"{f3(w['heldout']['CE_uni'], 2)} | {f3(w['heldout']['CE_bi'], 2)} | {f3(w['heldout']['I'])} | {f3(w['heldout']['ratio'])} | {f3(w['heldout_shuffled']['I'])} | "
