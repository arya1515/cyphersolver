"""Decrypt Cardinal Alessandrino's 1567 ciphers to the nuncio in Spain (ASV Segr. Stato Spagna 1, DECODE R85-R90).
...
def main():
    files = sys.argv[1:] or sorted(glob.glob(os.path.join(HERE, 'decode', 'DOC_R*_D1[6]*.txt')) +
                                   [] + glob.glob(os.path.join(HERE, 'decode', 'DOC_R115_D1*.txt')),
