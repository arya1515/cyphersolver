...
\begin{abstract}
John Armstrong, United States Minister to France, closed a private letter to Secretary of State James Madison of 30 August 1808 with a postscript of 49 groups of a numerical word-and-syllable code. The postscript is printed undeciphered in \emph{Founders Online} and listed as unsolved by Tomokiyo, who read about a third of its groups from one letter with known plaintext. The code itself, a 1,600-element system in use at the Paris legation from 1804 to 1810 and conventionally labelled by its group for \emph{the}, 972, was never published. We reconstruct it from a source not previously used for this purpose: the State Department's own despatch books on National Archives microfilm M34, where a clerk pencilled the decode above every group of a long despatch of October 1806. An image score for faint pencil locates the annotated frames; some 580 groups are recovered with a confidence grade for each, and the alphabetical layout of the code fixes most of the remainder. The postscript reads in full: Armstrong privately rates Jonathan Russell above every other candidate for the Paris consulate and dismisses two Irish-born rivals. A second coded letter of 22 February 1808 serves as an independent check. The table is released for the roughly forty other coded Armstrong despatches, most never read.
\end{abstract}
...
Armstrong's despatches from Paris between 1804 and 1810 use a single large code in which \grp{972} = \emph{the}. Weber notes some forty letters in it among the Despatches from United States Ministers to France (National Archives, Record Group 59, microcopy M34, rolls 13--14; \citealt{Weber:1979}). It is not the code of Armstrong's letter of 20 February 1808, a distinct system that remains unsolved. As recovered, the code has the following structure.
...
\paragraph{Homophones and nulls.} Common elements have several groups: \emph{re} is \grp{555}, \grp{561}, \grp{562}, \grp{869} and \grp{871} among others. \grp{1394} carries a pencil dash in the decodes and functions as a null or punctuation mark; it opens the postscript.
...
Group by group (grade in brackets): \grp{1394} null [H]; \grp{1116} \emph{s} [H]; \grp{1273} \emph{Rus-} \todo{confirm grade and frame}; \grp{250} \emph{ought} [I: between \grp{249} \emph{other} and \grp{252} \emph{own}]; \grp{1165} \emph{to} [H]; \grp{1405} \emph{be} [H]; \grp{972} \emph{the} [C]; \grp{148} \emph{consul} [I]; \grp{1459} \emph{he} [H]; \grp{1482} \emph{is} [H]; \grp{1201} \emph{a} [H]; \grp{821} \emph{n} [H]; \grp{130} \emph{America} [I: run boundary]; \grp{821} \emph{n} [H]; \grp{1429} \emph{by} [H]; \grp{720} \emph{bir} [I]; \grp{970} \emph{th} [H]; \grp{992} \emph{qua} [I]; \grp{1319} \emph{li} [H]; \grp{1048} \emph{fie} [I]; \grp{584} \emph{than} [H]; \grp{687} \emph{any} [H]; \grp{249} \emph{other} [H]; \grp{736} \emph{can} [H]; \grp{1013} \emph{di} [H]; \grp{750} \emph{da} [H]; \grp{967} \emph{te} [H]; \grp{1202} \emph{above} [I]; \grp{1561} \emph{men} [H]; \grp{927} \emph{him} [H]; \grp{1090} \emph{in} [H]; \grp{1052} \emph{fit} [I]; \grp{832} \emph{ness} [H]; \grp{934} \emph{o} [H]; \grp{510} \emph{mea} [H]; \grp{860} \emph{ly} [H]; \grp{1320} \emph{like} [H: frame 0190]; \grp{384} \emph{Warden} \todo{grade}; \grp{1280} \emph{en} [H]; \grp{1216} \emph{an} [H]; \grp{1481} \emph{ir} [H]; \grp{1483} \emph{ish} \todo{grade}; \grp{1555} \emph{man} [H, reading the manuscript's \grp{555} as a dropped digit].

\todo{Table 2: the 49 groups in a three-column table (group, value, grade, source frame).}

\paragraph{Historical sense.} Jonathan Russell (Armstrong's spelling \emph{Russel}), Rhode Island merchant and later Minister to Sweden, was indeed appointed to the Paris consular post in 1810 \todo{verify date and source}. Warden, born in Ireland, held the consulate provisionally and lost it. ``O'Mealy'' is the Irish-born merchant known to the Madison correspondence from a letter of John Mitchell of 4 November 1805 \todo{cite Founders document 02-10-02-0473}. That Armstrong reserved this ranking for code, while treating Warden neutrally in clear, is exactly the pattern of use one expects of a diplomatic code: the text is ordinary, and the secret is an opinion about persons.
