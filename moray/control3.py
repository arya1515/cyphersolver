...
from solve3 import run
ho=open('corpus/historielifeofki00colvuoft.txt',encoding='utf-8',errors='ignore').read()
txt=re.sub(r' +','',clean(ho))
rnd=random.Random(int(sys.argv[1])); N=134; nh=int(sys.argv[3]) if len(sys.argv)>3 else 6
accs=[]
for trial in range(int(sys.argv[2])):
    ...
    # uneven homophone use: main symbol with prob .6, others share .4
    def enc(c):
        h=syms[c]
        if len(h)==1 or rnd.random()<0.6: return h[0]
        return rnd.choice(h[1:])
    ct=[enc(c) for c in p]
    res=run(ct,12,verbose=False)
    dec=res[0][1]; acc=sum(a==b for a,b in zip(dec,p))/N; accs.append(acc)
    print(f'trial {trial} distinct={len(set(ct))} acc={acc:.2f} score={res[0][0]:.1f}\n  P {p}\n  D {dec}',flush=True)
