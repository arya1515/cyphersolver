"""Candidate-only K search in observed R1101 writing regions."""
import json
...
for name,(filename,regions) in jobs.items():
    ...
    for scale in (0.8,1.,1.2):
        t=np.asarray(Image.fromarray(base.astype('float32')).resize((round(69*scale),round(20*scale))),float)
        t-=t.mean()
        h,w=t.shape
    ...
    sheet.save(ROOT/f'img/k_search_{name}.jpg')
(ROOT/'k_r1101_candidates.json').write_text(json.dumps(result,indent=2)+'\n')
