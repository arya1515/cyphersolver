"""Candidate-only emperor-expression search in observed R1101 writing regions."""
import json
...
ref=Image.open(ROOT/'img/IMG_R1101_I5654_P1.png').resize((1080,810))
base=ink(ref)[275:290,235:275]
jobs={
 'I5654':('IMG_R1101_I5654_P1.png',[(180,700,2130,2950),(2280,430,4040,1070),(2260,2200,4140,3020)]),
 'first':('v/v1284_1_ASMO Ambasciatori Busta 1 (103).JPG',[(1200,180,3330,2850)])
}
result={}
for name,(filename,regions) in jobs.items():
    ...
    for scale in (0.8,1.,1.2):
        t=np.asarray(Image.fromarray(base.astype('float32')).resize((round(40*scale),round(15*scale))),float)
        t-=t.mean()
        h,w=t.shape
    ...
    sheet.save(ROOT/f'img/emperor_search_{name}.jpg')
(ROOT/'emperor_r1101_candidates.json').write_text(json.dumps(result,indent=2)+'\n')
