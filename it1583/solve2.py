...
A='abcdefghilmnopqrstuxz_'
CAP=dict(_=4,a=3,e=3,i=3,o=3,u=2,n=2,r=1,l=2,s=2,t=2,c=2,d=1,p=1,m=1,g=1,h=1,b=1,f=1,q=1,x=1,z=1)
def text(key): return ''.join(key[x] if k=='c' else x for k,x in seq).replace('_','')
...
for rs in range(int(sys.argv[1]) if len(sys.argv)>1 else 8):
    ...
    cur=score(key); T=3.0
    for it in range(int(os.environ.get('IT','60000'))):
