"""View line strips of f.98 at readable magnification, stacked."""
import sys, os
from PIL import Image, ImageDraw
out=sys.argv[1]; nums=[int(x) for x in sys.argv[2:]]
SC=float(os.environ.get('SC','0.80'))
parts=[]
for n in nums:
    im=Image.open(f'L98/L{n:02d}.png').convert('L')
    im=im.resize((int(im.size[0]*SC),int(im.size[1]*SC)),Image.LANCZOS)
    parts.append((f'L{n}',im))
