...
out=sys.argv[1]; d=sys.argv[2]; nums=[int(x) for x in sys.argv[3:]]
SC=float(os.environ.get('SC','0.78'))
parts=[]
for n in nums:
    im=Image.open(f'{d}/D{n:02d}.png').convert('L')
...
for t,p in parts:
    dr.text((3,y+3),t,fill=0); s.paste(p,(0,y+18)); y+=p.size[1]+18
