"""Fetch a page region at high resolution."""
import sys, os, urllib.request, time
ARKS={'3983':'btv1b9059406b','3984':'btv1b9060633d'}
vol=os.environ.get('VOL','3983'); ARK=ARKS[vol]
UA={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0 Safari/537.36'}
REG=os.environ.get('REG','full'); W=int(os.environ.get('W','3000'))
tag=REG.replace(':','').replace(',','_')
os.makedirs(f'img{vol}',exist_ok=True)
for a in sys.argv[1:]:
    i=int(a); fn=f'img{vol}/r{i:04d}_{tag}_w{W}.jpg'
    ...
    for t in range(5):
        try:
            ...
            open(fn,'wb').write(d); print(i,len(d),fn); break
        except Exception as e:
            print(i,'fail',str(e)[:60]); time.sleep(6*(t+1))
print('DONE')
