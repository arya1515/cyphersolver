...
import solve as B
LQ,FL=B.LQ,B.FL
V='a ai an au b be bi c ca ce ci co d da de di do du e em en er es et f fai fe g ga ge h hi i ia ie il ju k l la le m ma me n na ne ni no nu o oi on ou p pour pr q que qui r ra re ri s sa se si son t te u vous x y z'.split()
import os
if os.environ.get('EXTRA'): V+=os.environ['EXTRA'].split()
...
def run(seed,iters):
  ...
  if os.environ.get('INIT'):
    J=json.load(open(os.environ['INIT']))
    for s,sym in enumerate(syms): key[s]=V.index(J[sym])
  inv={}
  for s in range(n): inv[key[s]]=s
  SS=[segsc(key,k) for k in range(len(segsI))]; cur=sum(SS); best=(cur,key[:])
  for it in range(iters):
    T=max(0.05, float(os.environ.get('T0','8'))*(1-it/iters))
...
json.dump({s:V[k] for s,k in zip(syms,key)},open(os.environ.get('OUT',f'perm_{seed}.json'),'w'))
