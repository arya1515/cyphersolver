...
def run(segsI,nsym,rounds=30,seed=0,fixed=None,init=None,W=1.0,restarts=5,log=False):
  ...
  for r in range(restarts):
    if init and r==0: key=list(init)
      ...
      key=best[1][:]
      for s in rnd.sample(free,max(1,len(free)//4)): key[s]=rnd.choice('esaitnrulodc')
