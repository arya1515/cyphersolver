...
for k in sorted(set(lab.tolist())):
    ...
    tpl[k]=acc/len(idx)
def explain(box, maxk=3, thr=None):
    h,w=box.shape; cands=[]
    for k,T in tpl.items():
        ...
        if ncc[y,x]>(thr.get(k,0.5) if thr else 0.5): cands.append((float(ncc[y,x])*np.sqrt(T.sum()),float(ncc[y,x]),k,int(x),int(y),W,H))
...
THR={}
for k,T in tpl.items():
    idx=[i for i in range(len(meta)) if lab[i]==k]; H,W=T.shape; Tz=T-T.mean(); e=np.sqrt((Tz**2).sum()); ss=[]
    for i in idx:
    ...
    THR[k]=max(0.5,float(np.percentile(ss,15))-0.03) if ss else 0.6
print('thr median',round(float(np.median(list(THR.values()))),2))
...
for i,m in enumerate(meta):
    ...
    nmax=int(max(1,min(3,round((m['x1']-m['x0'])/clean_w))))
    acc,expl=explain(B[y0:y1,x0:x1].copy(),maxk=nmax,thr=THR)
    if not acc or expl<0.55: new.append((m['line'],m['x0'],-1,0.0)); stats['gap']+=1; continue
...
pickle.dump(new,open('f122r_local3.pkl','wb'))
